cov_data <- read.csv2("ü6/covid_19_daily_reports_11-21-2021.csv", header = TRUE)
times_data <- read.table("ü6/time_series_covid19_confirmed_11-21-2021.csv", sep = " ", header = TRUE)

cov_data_ger <- subset(cov_data, Country_Region == "Germany")

#============== B) =============

aridmidconf <- function() {

  x <- 0;
  for (i in seq_len(nrow(cov_data_ger))) {
    x <- x + cov_data_ger$Confirmed[i]
  }
  return(x / nrow(cov_data_ger))
}

aridmiddeath <- function() {

  x <- 0;
  for (i in seq_len(nrow(cov_data_ger))) {
    x <- x + cov_data_ger$Deaths[i]
  }
  return(x / nrow(cov_data_ger))
}
